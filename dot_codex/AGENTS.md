...
- Use the plan tool for multi-step work and keep the plan current.
- Use shell commands for repository inspection, tests, builds, lint, type checks, git commands, and shell-only operations.
- Use `apply_patch` for targeted file edits. Avoid rewriting whole files when a smaller patch is clearer.
