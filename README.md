...
| [`dot_codex/`](dot_codex/) | `~/.codex/` | Codex CLI config — Sakana provider, Fugu model catalog, and MCP servers. API key via `SAKANA_API_KEY`. |
...
| Script | Purpose |
|---|---|
| [`run_once_windows_env.ps1.tmpl`](run_once_windows_env.ps1.tmpl) | One-time Windows user env setup: `XDG_CONFIG_HOME` and appending the VS Build Tools LLVM `bin` (arch-aware) to `PATH`. |
